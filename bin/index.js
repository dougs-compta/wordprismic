...
(async () => {
    ...
    const users = await getAllWp('users'),
        ...
        featuredMedias = optimizeMediaRequests
            ? await getAllWp('media', `include=${posts.map((post) => post.featured_media).join(',')}`)
            : await getAllWp('media'),
        parsePost = async (post) => {
            const { featured_media, author, categories } = post;

            Object.assign(post, {
                featured_media: featuredMedias.find((media) => media.id === featured_media),
                author: users.find((user) => user.id === author),
                categories: categories.map((category) => topics.find((topic) => topic.wordpress.id === category))
            });
    ...
    for (const post of posts) {
        try {
            await writePost(await parsePost(post));
        } catch (e) {
            console.log(chalk.red(`Failed to convert post ${post.slug}:`));
            console.log(e);
        }
    }

    console.log(
        chalk.green(`Finished! Compress the folder at ${dest}${OUTPUT_FOLDER} into a zip and import to Prismic`)
    );
})();
