...
async function getAllWp(resource, queryParams) {
  ...
  const baseQuery = `${WP_API}/${resource}?per_page=100&${queryParams}`,
    totalPages = await fetch(baseQuery, { mode: "headers", headers }).then(
      (response) => response.headers.get("x-wp-totalpages")
    ),
    getPage = async (page) => {
      console.log(
        chalk.gray(
      ...
      return await fetch(`${baseQuery}&page=${page}`, { headers }).then(
        (response) => {
          if (!response.ok) {
            throw new Error(
...
async function htmlParser(html) {
  ...
  return await new Promise((resolve, reject) => {
    exec(
      `html2prismic ${JSON.stringify(fixedHtml.replace(/\r?\n|\r/g, ""))}`,
      (err, stdout) => {
        err && reject(err);
