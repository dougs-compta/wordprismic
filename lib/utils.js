...
async function getAllWp(resource, queryParams) {
    ...
    const baseQuery = `${WP_API}/${resource}?per_page=100&${queryParams}`,
        totalPages = await fetch(baseQuery, { mode: 'headers', headers }).then((response) =>
            response.headers.get('x-wp-totalpages')
        ),
        getPage = async (page) => {
            console.log(chalk.gray(`Fetching ${page > 1 ? `page ${page} of ` : ``}${resource}...`));
            return await fetch(`${baseQuery}&page=${page}`, { headers }).then((response) => {
                if (!response.ok) {
...
async function htmlParser(html) {
    ...
    return await new Promise((resolve, reject) => {
        exec(`html2prismic ${JSON.stringify(fixedHtml.replace(/\r?\n|\r/g, ''))}`, (err, stdout) => {
            err && reject(err);
...
async function mapCategories() {
    const prismicApi = await Prismic.getApi(`https://${prismic.repo}.prismic.io/api/v2`),
        prismicTopics = await prismicApi.query(Prismic.Predicates.at('document.type', prismic.categoriesType), {
            pageSize: 100
        }),
        wpCategories = await getAllWp('categories');

    return wpCategories.map((wpCategory) => {
        const prismicTopic = prismicTopics.results.find((topic) => wpCategory.slug === topic.uid);
